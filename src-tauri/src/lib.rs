...
fn build_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    // Bamboo silhouette as the tray icon. Generated by
    // `scripts/make-tray-icon.mjs` into `icons/tray.png` and embedded
    // here at compile time. icon_as_template(true) tells macOS to treat
    // it as a template image — black pixels become the menu bar's
    // foreground color (black in light mode, white in dark mode), so it
    // always reads cleanly. Without an explicit icon, Tauri silently
    // falls back to the full-color app icon and that renders as a small
    // white-ish square next to the percentage.
